...
getFluo <- function(H5_path, Spots_df, x_px = x_px, y_px = y_px, z_px = z_px,
  ...
  all_cells <- sapply(unique(as.numeric(Spots_df$FRAME)), function(t){
    ...
    cells <- lapply(S, function(coords, H5_path) {
      ...
      pixels <- rhdf5::h5read(file = H5_path, name = glue("/t00{t}/s00/0/cells"), #for small example, green channel in s01; check how are channels in bioutifoul
