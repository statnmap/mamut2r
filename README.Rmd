...
This should install every dependencies needed
```{r}
# Install with dependencies
remotes::install_github("marionlouveaux/mamut2r", dependencies = TRUE)
```

{rhdf5} is *only needed if* you want to extract fluorescence intensity from HDF5 files as presented in this vignette: <https://marionlouveaux.github.io/mamut2r/articles/B_fluo.html>
*Note that last version of {rhdf5} requires R>=3.5*
...
to_install <- c("classInt", "cowplot", "dplyr", "ggplot2", "ggraph", "ggridges", "glue", "grDevices", "igraph", "knitr", "magrittr", "pkgdown", "purrr", "rmarkdown", "stats", "tibble", "tidyr", "utils", "viridis", "XML", "xml2")
for (i in to_install) {
...
You need to have Rtools installed to be able to install this package from Github. Rtools is not a R package, this a *collection of resources for building packages for R under Microsoft Windows*.
You may encounter some problems with Rtools installation, if so, please follow these instructions:
